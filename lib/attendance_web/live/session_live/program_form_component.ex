defmodule AttendanceWeb.SessionLive.ProgramFormComponent do
  ...
  defp save_program(socket, :edit_program, program_params) do
  ...
  defp save_program(socket, :new_program, program_params) do
    session = Catalog.get_session!(program_params["session_id"])
    case Catalog.create_program(socket.assigns.current_admin, session, program_params) do
      {:ok, _program} ->
        {:noreply,
         socket
